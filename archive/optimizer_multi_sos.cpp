...
void Optimizer::optimize() { 
  ...
  //###############################################################################  drone dependent variable  #############################################################################
    ...
    std::vector<GRBVar> sensor_pix_selector(num_drones * feasible_pix_indices.size());
    ...
    std::vector<GRBVar> drone_set_fps_selector(num_drones * feasible_fps_indices.size());
    ...
    for (int drone = 0; drone < num_drones; drone++) {
      ...
      drone_v_true[drone] = model.addVar(SPEED_MIN, SPEED_MAX, 0.0, GRB_CONTINUOUS, "drone_v_true_" + std::to_string(drone)); // true velocity of the drone
      drone_v_true_inv[drone] = model.addVar(1/SPEED_MAX, 1/SPEED_MIN, 0.0, GRB_CONTINUOUS, "drone_v_true_inv_" + std::to_string(drone)); // inverse of true velocity of the drone
      ...
      //sensor_fps_true[drone] = model.addVar(FPS_MIN, FPS_MAX, 0.0, GRB_CONTINUOUS, "sensor_fps_true_" + std::to_string(drone)); // true fps
      ...
      for (int idx = 0; idx < feasible_pix_indices.size(); ++idx) {
          int i = feasible_pix_indices[idx];
          sensor_pix_selector_total[drone] += sensor_pix_selector[drone * feasible_pix_indices.size() + idx];
          sensor_pix_x_value[drone] += drone_set_pix_x[i] * sensor_pix_selector[drone * feasible_pix_indices.size() + idx];
          sensor_pix_y_value[drone] += drone_set_pix_y[i] * sensor_pix_selector[drone * feasible_pix_indices.size() + idx];
          drone_set_pix_value[drone] += drone_set_pix_normalized[i] * sensor_pix_selector[drone * feasible_pix_indices.size() + idx];
      }
      ...
      for (int idx = 0; idx < feasible_fps_indices.size(); ++idx) {
          int i = feasible_fps_indices[idx];
          drone_set_fps_selector_total[drone] += drone_set_fps_selector[drone * feasible_fps_indices.size() + idx];
          drone_set_fps_value[drone] += drone_set_fps_normalized[i] * drone_set_fps_selector[drone * feasible_fps_indices.size() + idx];
      }
      ...
      covered_area_total_t0[drone] = model.addVar(0.0, COVERED_AREA_TOTAL_MAX, 0.0, GRB_CONTINUOUS, "covered_area_total_t0_" + std::to_string(drone));
      ...
      covered_area_total[drone] = model.addVar(0.0, FIELD_AREA, 0.0, GRB_CONTINUOUS, "covered_area_total_" + std::to_string(drone));
      ...
      covered_distance[drone] = model.addVar(0.0, COVERED_DISTANCE_MAX, 0.0, GRB_CONTINUOUS, "covered_distance_" + std::to_string(drone)); // distance covered by the drone
      ...
      // power true
      drone_pa_consumption[drone] = model.addVar(0.0, 100.0, 0.0, GRB_CONTINUOUS, "drone_pa_consumption_" + std::to_string(drone)); // actuator power consumption for each drone
      drone_ps_consumption[drone] = model.addVar(0.0, 100.0, 0.0, GRB_CONTINUOUS, "drone_ps_consumption_" + std::to_string(drone)); // sensor power consumption for each drone
    ...
    objective_expr = 1*total_energy_consumed + 10*drone_used_total;
