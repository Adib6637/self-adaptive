...
void Optimizer::optimize() { 
  ...
  if (!OPTIMIZER_ON || counter == observed_data[19].read()|| observed_data[19].read() < 3 ||
  ...
  //####################################################################################### Constants ###################################################################################
    ...
    double pa_C_0 = pa_theta * (((GRAVITY-(LINEAR_ACCELERATION_Z_MAX*(-1)))/((LINEAR_ACCELERATION_Z_MIN*(-1))-(LINEAR_ACCELERATION_Z_MAX*(-1)))) * ((DRONE_MASS)/MASS_MAX)) * pa_phi + pa_theta*pa_alpha*std::pow(pa_phi,3) + pa_delta;
    double pa_C_1 = SCALE_INV*(pa_theta * (((GRAVITY-(LINEAR_ACCELERATION_Z_MAX*(-1)))/((LINEAR_ACCELERATION_Z_MIN*(-1))-(LINEAR_ACCELERATION_Z_MAX*(-1)))) * ((DRONE_MASS)/MASS_MAX)) + 3*pa_theta*pa_alpha*std::pow(pa_phi,2));
    double pa_C_2 = SCALE_INV*(3 * pa_theta * pa_alpha * pa_phi);
    double pa_C_3 = SCALE_INV*(pa_theta * pa_alpha);
    double pa_C_4 = pa_theta * pa_beta * ((DRONE_MASS)/MASS_MAX);
  ...
  //######################################################################################  Constraint  ####################################################################################
    ...
    for (int drone = 0; drone < NUMBER_DRONE_MAX; drone++) {
      ...
      // speed
      drone_v[drone] = model.addVar(0.0, SCALE, 0.0, GRB_INTEGER, "drone_v_" + std::to_string(drone));
      drone_v_2[drone] = model.addVar(0.0, (SCALE*SCALE), 0.0, GRB_INTEGER, "drone_v_2_" + std::to_string(drone));
      drone_v_3[drone] = model.addVar(0.0, (SCALE*SCALE*SCALE), 0.0, GRB_INTEGER, "drone_v_3_" + std::to_string(drone));
      drone_v_true[drone] = model.addVar((0.5*SCALE), (5*SCALE), 0.0, GRB_INTEGER, "drone_v_true_" + std::to_string(drone)); 
      ...
      model.addQConstr(drone_v_true[drone] == (drone_v[drone] * ( (SPEED_MAX) -  (SPEED_MIN) )) + (SPEED_MIN*SCALE), "drone_v_true_identity_" + std::to_string(drone)); 
      model.addQConstr(drone_v_true[drone] <= (MAX_V_CAPTURING*SCALE), "coverage_fps_lower_bound_" + std::to_string(drone)); 
      //model.addQConstr(drone_v_true[drone] <= sensor_fps_true[drone]*CONST_2_TAN_CAMERA_THETA_OVERLAP_FACTOR, "coverage_fps_lower_bound_" + std::to_string(drone)); 

      // height
      drone_h[drone] = model.addVar(0.0, SCALE, 0.0, GRB_INTEGER, "drone_h_" + std::to_string(drone));
      model.addQConstr(drone_h[drone]* ( ALTITUDE_MAX - ALTITUDE_MIN ) + (ALTITUDE_MIN*SCALE) >= (1.5*SCALE) , "drone_h_identity_" + std::to_string(drone)); 
      model.addQConstr(drone_h[drone]* ( ALTITUDE_MAX - ALTITUDE_MIN ) + (ALTITUDE_MIN*SCALE) <= (11.0*SCALE) , "drone_h_identity_" + std::to_string(drone)); 
      ...
      model.addQConstr(operation_time[drone]* drone_v_true[drone] >= covered_distance[drone]*SCALE , "operation_time_identity_" + std::to_string(drone)); 
    ...
    if (model_status == GRB_OPTIMAL) {
      ...
      for (int drone = 0; drone < NUMBER_DRONE_MAX; drone++) {
        if(drone_is_used[drone].get(GRB_DoubleAttr_X)){
          ...
          PRINT_LOG(25, "v_true ", (drone_v_true[drone].get(GRB_DoubleAttr_X)/SCALE))
          PRINT_LOG(25, "h_true", (drone_h[drone].get(GRB_DoubleAttr_X))* ( ALTITUDE_MAX - ALTITUDE_MIN ) + ALTITUDE_MIN)
          ...
          csv << counter << "," << drone << ","
              ...
              << drone_v_true[drone].get(GRB_DoubleAttr_X)*drone_is_used[drone].get(GRB_DoubleAttr_X)/SCALE << ","
