...
void Optimizer::optimize() { 
  ...
  //###############################################################################  drone dependent variable  #############################################################################
    ...
    // Pre-filter infeasible pixel/fps configurations to reduce selector variables
    std::vector<int> feasible_pix_indices;
    for (int i = 0; i < drone_set_pix.size(); ++i) {
        // Example: Only keep pixel configs within a tighter range (customize as needed)
        if (drone_set_pix_x[i] >= PIX_X_MIN && drone_set_pix_x[i] <= PIX_X_MAX &&
            drone_set_pix_y[i] >= PIX_Y_MIN && drone_set_pix_y[i] <= PIX_Y_MAX) {
            feasible_pix_indices.push_back(i);
        }
    }
    std::vector<int> feasible_fps_indices;
    for (int i = 0; i < drone_set_fps.size(); ++i) {
        // Example: Only keep fps configs within a tighter range (customize as needed)
        if (drone_set_fps[i] >= FPS_MIN && drone_set_fps[i] <= FPS_MAX) {
            feasible_fps_indices.push_back(i);
        }
    }

    // pixel x and y selector (reduced set)
    std::vector<GRBVar> sensor_pix_selector(num_drones * feasible_pix_indices.size());
    std::vector<GRBLinExpr> sensor_pix_selector_total(num_drones, 0);
    std::vector<GRBLinExpr> sensor_pix_x_value(num_drones, 0);
    std::vector<GRBLinExpr> sensor_pix_y_value(num_drones, 0);
    std::vector<GRBLinExpr> drone_set_pix_value(num_drones, 0);
    // fps selector (reduced set)
    std::vector<GRBVar> drone_set_fps_selector(num_drones * feasible_fps_indices.size());
    std::vector<GRBLinExpr> drone_set_fps_selector_total(num_drones, 0);
    std::vector<GRBLinExpr> drone_set_fps_value(num_drones, 0);
    ...
    for (int drone = 0; drone < num_drones; drone++) {
      ...
      covered_area_true[drone] = model.addVar(0.0, 1000000.0, 0.0, GRB_CONTINUOUS, "covered_area_true_" + std::to_string(drone)); // total area covered by the drone
    ...
    objective_expr = 1*total_energy_consumed + 100*drone_used_total;
