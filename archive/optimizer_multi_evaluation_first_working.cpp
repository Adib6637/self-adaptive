...
void Optimizer::optimize() { 
  ...
  //###############################################################################  drone dependent variable  #############################################################################
    ...
    // Pre-filter infeasible pixel/fps configurations (example: only allow configs with fps*pix within a range)
    /*
    std::vector<int> feasible_pix_indices;
    for (int i = 0; i < drone_set_pix.size(); ++i) {
        // Example filter: only allow pixel configs above a threshold (customize as needed)
        if (drone_set_pix[i] >= PIXELS_MIN && drone_set_pix[i] <= PIXELS_MAX) {
            feasible_pix_indices.push_back(i);
        }
    }
    std::vector<int> feasible_fps_indices;
    for (int i = 0; i < drone_set_fps.size(); ++i) {
        if (drone_set_fps[i] >= FPS_MIN && drone_set_fps[i] <= FPS_MAX) {
            feasible_fps_indices.push_back(i);
        }
    }*/
    ...
    for (int drone = 0; drone < num_drones; drone++) {
      ...
      // pixel x and y selector (SOS1)
      /*
      std::vector<GRBVar> pix_vars;
      for (int idx = 0; idx < feasible_pix_indices.size(); ++idx) {
          int i = feasible_pix_indices[idx];
          sensor_pix_selector[drone * feasible_pix_indices.size() + idx] = model.addVar(0.0, 1.0, 0.0, GRB_CONTINUOUS, "sensor_pix_selector_" + std::to_string(drone) + "_" + std::to_string(i));
          pix_vars.push_back(sensor_pix_selector[drone * feasible_pix_indices.size() + idx]);
      }
      if (!pix_vars.empty()) {
        std::vector<double> sos_weights(pix_vars.size());
        for (size_t i = 0; i < sos_weights.size(); ++i) sos_weights[i] = static_cast<double>(i);
        model.addSOS(pix_vars.data(), sos_weights.data(), pix_vars.size(), GRB_SOS_TYPE1);
      }
      */
      for (int i = 0; i < drone_set_pix.size(); i++) {
          //int i = feasible_pix_indices[idx];
      ...
      // fps selector (SOS1)
      /*
      std::vector<GRBVar> fps_vars;
      for (int idx = 0; idx < feasible_fps_indices.size(); ++idx) {
          int i = feasible_fps_indices[idx];
          drone_set_fps_selector[drone * feasible_fps_indices.size() + idx] = model.addVar(0.0, 1.0, 0.0, GRB_CONTINUOUS, "drone_set_fps_selector_" + std::to_string(drone) + "_" + std::to_string(i));
          fps_vars.push_back(drone_set_fps_selector[drone * feasible_fps_indices.size() + idx]);
      }
      if (!fps_vars.empty()) {
        std::vector<double> sos_weights(fps_vars.size());
        for (size_t i = 0; i < sos_weights.size(); ++i) sos_weights[i] = static_cast<double>(i);
        model.addSOS(fps_vars.data(), sos_weights.data(), fps_vars.size(), GRB_SOS_TYPE1);
      }
      */
      for (int i = 0; i < drone_set_fps.size();i++) {
          //int i = feasible_fps_indices[idx];
      ...
      covered_area_total_t0[drone] = model.addVar(0.0, COVERED_AREA_TOTAL_MAX, 0.0, GRB_SEMICONT, "covered_area_total_t0_" + std::to_string(drone));
      ...
      covered_area_total[drone] = model.addVar(0.0, FIELD_AREA, 0.0, GRB_SEMICONT, "covered_area_total_" + std::to_string(drone));
      ...
      number_of_place_covered[drone] = model.addVar(NUMBER_PLACE_COVERED_MIN, NUMBER_PLACE_COVERED_MAX, 0.0, GRB_SEMICONT, "number_of_place_covered_" + std::to_string(drone)); // number of place covered by the drone
      //distance_between_place[drone] = model.addVar(0.0, COVERED_AREA_X_MAX, 0.0, GRB_CONTINUOUS, "distance_between_place_" + std::to_string(drone)); // distance between place covered by the drone
      //covered_area_total_inv[drone] = model.addVar(1/FIELD_AREA, GRB_INFINITY, 0.0, GRB_CONTINUOUS, "covered_area_total_inv_" + std::to_string(drone)); // inverse of total area covered by the drone
      covered_distance[drone] = model.addVar(COVERED_AREA_X_MIN, COVERED_DISTANCE_MAX, 0.0, GRB_SEMICONT, "covered_distance_" + std::to_string(drone)); // distance covered by the drone
      operation_time[drone] = model.addVar(OPERATION_TIME_MIN, GRB_INFINITY, 0.0, GRB_SEMICONT, "operation_time_" + std::to_string(drone)); // operation time of the drone   
      ...
      // power true
      //drone_pa_consumption[drone] = model.addVar(POWER_ACTUATOR_MIN, POWER_ACTUATOR_MAX, 0.0, GRB_SEMICONT, "drone_pa_consumption_" + std::to_string(drone)); // actuator power consumption for each drone
      ...
      drone_ps_consumption[drone] = model.addVar(POWER_SENSOR_MIN, POWER_SENSOR_MAX, 0.0, GRB_SEMICONT, "drone_ps_consumption_" + std::to_string(drone)); // sensor power consumption for each drone
      drone_energy_consumption[drone] = model.addVar(0.0, 10000000.0, 0.0, GRB_SEMICONT, "drone_energy_consumption_" + std::to_string(drone)); // energy consumption for each drone
      ...
      operation_time_req[drone] = model.addVar(OPERATION_TIME_MIN, GRB_INFINITY, 0.0, GRB_SEMICONT, "operation_time_req_" + std::to_string(drone)); // total operation time including charging time
      ...
      charging_cycles[drone] = model.addVar(0.0, 20.0, 0.0, GRB_SEMIINT, "charging_cycles_" + std::to_string(drone)); // number of charging cycles for each drone
      model.addQConstr(charging_cycles[drone] == (operation_time[drone] - OPERATION_MAX_PER_CHARGING)*OPERATION_MAX_PER_CHARGING_INV);
      model.addQConstr(operation_time_req[drone] == operation_time[drone] + (charging_cycles[drone] * CHARGING_TIME), "operation_time_req_identity_" + std::to_string(drone)); // total operation time including charging time is equal to operation time plus charging time multiplied by number of charging cycles
  ...
  //####################################################################################  optimization  ##################################################################################
    ...
    model.set(GRB_DoubleParam_Heuristics, 0.1); //Spend 50% of time on heuristics
