...
void Optimizer::optimize() { 
  ...
  model.addGenConstrPow(drone_v, drone_v_2, 2.0, "drone_v_2_identity");
  model.addGenConstrPow(drone_v, drone_v_3, 3.0, "drone_v_3_identity");
  model.addGenConstrPow(drone_v, drone_v_inv, -1.0, "drone_v_inv_identity");
  model.addQConstr(drone_v_true == drone_v * (SPEED_MAX - SPEED_MIN)+ SPEED_MIN, "drone_v_true_identity"); // true velocity of the drone is equal to the normalized velocity multiplied by the maximum speed plus the minimum speed
  model.addGenConstrPow(drone_v_true, drone_v_true_inv, -1.0, "drone_v_true_inv_identity");
  model.addQConstr(sensor_fps_pix == sensor_fps * sensor_pix, "sensor_fps_pix_identity");
  model.addGenConstrPow(sensor_fps, sensor_fps_2, 2.0, "sensor_fps_2_identity");
  model.addQConstr(sensor_fps_true == sensor_fps*(FPS_MAX-FPS_MIN) + FPS_MIN, "sensor_fps_true_identity"); // fps == sensor_fps
  ...
  model.addGenConstrPow(sensor_pix, sensor_pix_2, 2.0, "sensor_pix_2_identity");
  model.addGenConstrPow(sensor_pix_x, sensor_pix_x_inv, -1.0, "sensor_pix_x_inv_identity");
  model.addGenConstrPow(sensor_pix_y, sensor_pix_y_inv, -1.0, "sensor_pix_y_inv_identity");
  ...
  model.addGenConstrPow(covered_area_x_t0, covered_area_x_t0_inv, -1.0, "covered_area_x_t0_inv_identity");
  model.addGenConstrPow(covered_area_y_t0, covered_area_y_t0_inv, -1.0, "covered_area_y_t0_inv_identity");
  model.addGenConstrPow(covered_area_total_t0, covered_area_total_t0_inv, -1.0, "covered_area_total_t0_inv_identity");
  ...
  model.addGenConstrPow(covered_area_total, covered_area_total_inv, -1.0, "covered_area_total_inv_identity");
  model.addQConstr(covered_area_y_t0*sensor_pix_y_inv <= 0.01, "covered_area_x_t0_resolution_min");
  model.addQConstr(covered_area_x_t0 == CONST_2_TAN_CAMERA_THETA * (drone_h*(ALTITUDE_MAX - ALTITUDE_MIN)+ ALTITUDE_MIN), "covered_area_x_t0_identity"); // area covered in x direction is equal to 2 * tan(camera theta) * (drone height * maximum altitude + minimum altitude) / pixel x resolution
